...
let atomConfig = {
    ...
    codeHighlightingTheme: {
        type: 'string',
        default: 'github.css', //'github-gist.css',
        enum: (function () {
    ...
    customStylesPath: {
        ...
        description: 'You can use this stylesheet file to customize everything. The path is relative to Atom config directory <code>' + getAtomConfigDirPath() + '</code>. This relative path can also be used inside each project.',
        order: 150
    },
    enableCustomHeader: {
        type: 'boolean',
    ...
    customHeaderPath: {
        ...
        description: 'You can use this javascript file to customize the document header. The path is relative to Atom config directory <code>' + getAtomConfigDirPath() + '</code>. This relative path can also be used inside each project.',
        order: 170
    },
    enableCustomFooter: {
        type: 'boolean',
    ...
    customFooterPath: {
        ...
        description: 'You can use this javascript file to customize the document footer. The path is relative to Atom config directory <code>' + getAtomConfigDirPath() + '</code>. This relative path can also be used inside each project.',
        order: 190
    },
...
// Fake it like we're Atom
class Atom {
    constructor() {
        this.notifications = {
            addInfo: console.log,
            addError: console.error,
            addSuccess: console.log,
            addWarning: console.warn,
        }
        this.config = {
            get: this.getConfig
        }
        ...
        this.workspace = {
            observeTextEditors: function(command) { },
            open: function(a, b) { },
        }
        this.project = {
            getPaths: function() {
    ...
    getConfig(name) {
        const m = /markdown-themeable-pdf\.(.*)/.exec(name);
        if (m) {
            return atomConfig[m[1]].default;
        }
        if (name = 'core.fileEncoding') {
            return 'UTF-8'
        }
        return undefined;
    }
...
let atomExports = markdownThemeablePdf = {
    config: atomConfig,
    subscriptions: null,
    activate: function () {
...
atomExports.activate();

module.exports = function(file) {
    atomExports.convertFile(file);
};
